...
from MuseControlLite.utils.extract_conditions import compute_melody, compute_melody_v2, compute_dynamics, extract_melody_one_hot, evaluate_f1_rhythm
from madmom.features.downbeats import DBNDownBeatTrackingProcessor,RNNDownBeatProcessor
import numpy as np
import matplotlib.pyplot as plt
import os
from MuseControlLite.utils.stable_audio_dataset_utils import load_audio_file
...
class StableAudioAttnProcessor2_0_rotary_double(torch.nn.Module):
    ...
    def __call__(
        ...
        return hidden_states
def setup_MuseControlLite(config, weight_dtype, transformer_ckpt, use_concept=False):
    """
    ...
    if 'audio' in config['condition_type'] and len(config['condition_type'])!=1:
        from MuseControlLite.pipeline.stable_audio_multi_cfg_pipe_audio import StableAudioPipeline
        from models.musecontrollite.stable_audio_multi_cfg_pipe_audio import MyStableAudioPipeline
        attn_processor = StableAudioAttnProcessor2_0_rotary_double
        print("Load stable_audio_multi_cfg_pipe_audio")
        audio_state_dict = load_file(config["audio_transformer_ckpt"], device="cpu")
    else:
        from MuseControlLite.pipeline.stable_audio_multi_cfg_pipe import StableAudioPipeline
        from models.musecontrollite.stable_audio_multi_cfg_pipe import MyStableAudioPipeline
        print("Load stable_audio_multi_cfg_pipe")
        attn_processor = StableAudioAttnProcessor2_0_rotary
    
    if use_concept:
        from models.musecontrollite.StableAudioDiTModel import MyStableAudioDiTModel
        ...
        from MuseControlLite.pipeline.StableAudioDiTModel import StableAudioDiTModel
        my_transformer = StableAudioDiTModel.from_pretrained(
            "stabilityai/stable-audio-open-1.0", 
            subfolder="transformer", 
            torch_dtype=weight_dtype
        )
...
def initialize_condition_extractors(config):
    ...
    extractor_ckpt = {}
    from MuseControlLite.utils.feature_extractor import dynamics_extractor, rhythm_extractor, melody_extractor_mono, melody_extractor_stereo, melody_extractor_full_mono, melody_extractor_full_stereo, dynamics_extractor_full_stereo
...
def build_inputs(config):
    if config["apadapter"]:
        assert len(config['audio_files']) > 0 and len(config['text']) == len(config['audio_files'])
    seeds = [100+i for i in range(len(config['text']))]
    prompts = config['text'] if not config['no_text'] else ["" for _ in range(len(config['text'] ))]
    concept_inputs = [1] * len(config['text']) if config['concept_cond'] else [0] * len(config['text'])
    audio_files = config["audio_files"] if config['apadapter'] else ["" for _ in range(len(prompts))]
    return prompts, audio_files, concept_inputs, seeds