...
from .StableAudioDiTModel import MyStableAudioDiTModel
...
class MyStableAudioPipeline(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        transformer: MyStableAudioDiTModel,
        scheduler: EDMDPMSolverMultistepScheduler,
    ):
        super().__init__()
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # duplicate steer for multi-CFG (uncond + cond_text + cond_audio)
                if steer is not None and do_classifier_free_guidance:
                    steer_model_input = torch.cat([steer]*3, dim=0)
                else:
