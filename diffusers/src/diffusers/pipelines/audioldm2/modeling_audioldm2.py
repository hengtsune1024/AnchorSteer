...
class AudioLDM2UNet2DConditionModel(ModelMixin, AttentionMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        edit_bottleneck_direction: Optional[torch.Tensor] = None,
        ...
        # [Modified]: Capture bottleneck activation
        if return_hidden:
            if do_classifier_free_guidance:
                bottleneck_activation = sample[sample.shape[0]//2:, ...]
            else:
                bottleneck_activation = sample
        
        if edit_bottleneck_direction is not None:
            if do_classifier_free_guidance:
                half = sample.shape[0] // 2
                print("edit midblock")
                
                # h = sample[half:]
                # w = edit_bottleneck_direction.to(h.dtype)

                # h_flat = h.view(half, -1)
                # w_flat = w.view(-1)

                # proj = (h_flat @ w_flat) / (h_flat.pow(2).sum(dim=1) + 1e-8)
                # w_orth = w_flat.unsqueeze(0) - proj.unsqueeze(1) * h_flat

                # sample[half:] = sample[half:] + w_orth.view_as(sample[half:])

                sample[half:] = sample[half:] + edit_bottleneck_direction
            else:
                sample = sample + edit_bottleneck_direction.unsqueeze(0)
        ...
        # [Modified]: Handle return types for hidden states
        if not return_dict:
            if return_hidden:
                return (sample, bottleneck_activation)
            return (sample,)
