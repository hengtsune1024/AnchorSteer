...
class AudioLDM2Pipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        edit_type: Optional[str] = None,        # edit_type in ['mid', 'lat', 'crs']
        edit_timestep: Optional[int | List] = None,    # most noisy => 0, 1, 2, 3, ..., => least noisy
        edit_direction: Optional[torch.Tensor] = None,
        ...
        if edit_timestep is not None:
            if isinstance(edit_timestep, int):
                edit_timestep = {edit_timestep}
            else:
                edit_timestep = {t for t in edit_timestep}
        ...
        # [Modified]: Variable to store bottleneck activation
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                unet_output = self.unet(
                    ...
                    edit_bottleneck_direction=(edit_direction[i] if edit_type == "mid" and i in edit_timestep else None)
                )
                noise_pred = unet_output[0]
                
                if return_hidden:
                    bottleneck_activation.append(unet_output[1])
        ...
        if not output_type == "latent":
            ...
            if edit_type == "lat":
                print("edit latent")
                orig_norm = latents.norm(p=2, dim=(1,2,3))
                latents = latents + edit_direction.to(latents.device)
                latents = latents / latents.norm(p=2, dim=(1,2,3)) * orig_norm
        ...
        if return_hidden:
            bottleneck_activation = torch.stack(bottleneck_activation, dim=0)   # (T, B, C, H, W)
            bottleneck_activation = bottleneck_activation.permute((1, 0, 2, 3, 4))  # (B, T, C, H, W)

        if not return_dict:
            if return_hidden:
                return (audio, prompt_embeds, generated_prompt_embeds, bottleneck_activation)
            return (audio,)

        # [Modified]: Construct return dictionary (or object) with hidden states
        output = AudioPipelineOutput(audios=audio)
        
        if return_hidden:
            B = bottleneck_activation.shape[0]
            output.latents = latents
            # output.prompt_embeds = prompt_embeds[B:] if do_classifier_free_guidance else prompt_embeds
            # output.generated_prompt_embeds = generated_prompt_embeds[B:] if do_classifier_free_guidance else generated_prompt_embeds
            output.bottleneck_activation = bottleneck_activation

        if return_mel:
            output.mel = mel_spectrogram
